  ...
  const handleOptimizeNesting = async () => {
    ...
    try {
      ...
      switch (selectedTechnology) {
        ...
        case 'binpacking':
          // Simulate a different algorithm with slightly different results
          results = CutOptimizer.optimizeNesting(
            currentConfiguration.cuttingList,
            sheetSize,
            selectedMaterial !== 'all' ? selectedMaterial : undefined
          );
          // Modify results slightly to simulate different algorithm
          results.forEach(result => {
            result.efficiency = Math.min(result.efficiency * 1.05, 99.5); // 5% better efficiency
        ...
        case 'd3js':
          // Simulate D3.js force-directed layout
          results = CutOptimizer.optimizeNesting(
            currentConfiguration.cuttingList,
            sheetSize,
            selectedMaterial !== 'all' ? selectedMaterial : undefined
          );
          // Modify results to simulate D3.js layout
          results.forEach(result => {
            result.efficiency = Math.max(result.efficiency * 0.98, 60); // Slightly less efficient
            // Rearrange parts in a more grid-like pattern
            let row = 0;
            let col = 0;
            const maxCol = Math.floor(result.sheetSize.length / 300);
            result.parts.forEach(part => {
              part.x = col * 300;
              part.y = row * 300;
              col++;
        ...
        case 'fabricjs':
          // Simulate Fabric.js canvas layout
          results = CutOptimizer.optimizeNesting(
            currentConfiguration.cuttingList,
            sheetSize,
            selectedMaterial !== 'all' ? selectedMaterial : undefined
          );
          // Modify results to simulate Fabric.js layout
          results.forEach(result => {
            result.efficiency = Math.min(result.efficiency * 1.02, 99); // Slightly better efficiency
            // Add more rotations to simulate Fabric.js flexibility
            result.parts.forEach(part => {
              if (Math.random() > 0.7) {
                part.rotation = part.rotation ? 0 : 90; // Toggle rotation
              }
            });
          });
          break;
        ...
        case 'cutlist':
          // Simulate specialized woodworking cut optimization
          results = CutOptimizer.optimizeNesting(
            currentConfiguration.cuttingList,
            sheetSize,
            selectedMaterial !== 'all' ? selectedMaterial : undefined
          );
          // Modify results to simulate CutList Optimizer
          results.forEach(result => {
            result.efficiency = Math.min(result.efficiency * 1.08, 99.9); // Better efficiency
            // Arrange parts in a more structured way
            let x = 0;
            let y = 0;
            let rowHeight = 0;
            result.parts.sort((a, b) => b.width - a.width); // Sort by width
            result.parts.forEach(part => {
              if (x + part.length > result.sheetSize.length) {
                x = 0;
                y += rowHeight;
                rowHeight = 0;
              }
              ...
              // Respect grain direction more strictly
              part.grainViolated = false;
            });
          });
          break;
        ...
        case 'webglrenderer':
        case 'reactkonva':
          // These are rendering technologies, not optimization algorithms
          // Use the base algorithm but with different visual properties
          results = CutOptimizer.optimizeNesting(
            currentConfiguration.cuttingList,
            sheetSize,
            selectedMaterial !== 'all' ? selectedMaterial : undefined
